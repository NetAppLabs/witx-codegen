...
impl DocValidationScope<'_> {
    ...
    fn validate_enum(
        ...
        Ok(Variant { tag_repr, tag_type, cases })
    }
    ...
    fn validate_union(
        ...
        let (tag_repr, _tag_type, names) = self.union_tag_repr(&syntax.tag, span)?;
        ...
        let tag_type = match &syntax.tag {
            Some(s) => { 
                match **s {
                    TypedefSyntax::Enum(_) => {None},
                    TypedefSyntax::Tuple(_) => {None},
                    TypedefSyntax::Expected(_) => {None},
                    TypedefSyntax::Flags(_) => {None},
                    TypedefSyntax::Record(_) => {None},
                    TypedefSyntax::Union(_) => {None},
                    TypedefSyntax::Variant(_) => {None},
                    TypedefSyntax::Handle(_) => {None},
                    TypedefSyntax::List(_) => {None},
                    TypedefSyntax::Pointer(_) => {None},
                    TypedefSyntax::ConstPointer(_) => {None},
                    TypedefSyntax::Builtin(_) => {None},
                    TypedefSyntax::Ident(i) => {
                        let wast_id = self.get(&i)?;
                        match self.doc.entries.get(&wast_id) {
                            Some(Entry::Typename(weak_ref)) => Some(TypeRef::Name(
                                weak_ref.upgrade().expect("weak backref to defined type"),
                            )),
                            _ => {None},
                        }                            
                    }
                    TypedefSyntax::String =>  {None},
                    TypedefSyntax::Bool => {None},
                }
            }
            None => {None}
        };
    ...
    fn validate_variant(
        ...
        let (tag_repr, tag_type, names) = self.union_tag_repr(&syntax.tag, span)?;
    ...
    fn union_tag_repr(
        ...
        span: wast::Span,
    ) -> Result<(IntRepr, Option<TypeRef>, Option<Vec<Id>>), ValidationError> {
        let ty = match tag {
            Some(tag) => self.validate_datatype(tag, false, span)?,
            None => return Ok((IntRepr::U32, None, None)),
        };
        match &**ty.type_() {
            Type::Variant(e) => {
                ...
                return Ok((e.tag_repr, e.tag_type.clone(), Some(names)));
            }
            Type::Builtin(BuiltinType::U8 { .. }) => return Ok((IntRepr::U8, None, None)),
            Type::Builtin(BuiltinType::U16) => return Ok((IntRepr::U16, None, None)),
            Type::Builtin(BuiltinType::U32 { .. }) => return Ok((IntRepr::U32, None, None)),
            Type::Builtin(BuiltinType::U64) => return Ok((IntRepr::U64, None, None)),
            _ => {}
        }
