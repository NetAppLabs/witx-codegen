...
impl<'a> Parse<'a> for UseSyntax<'a> {
    fn parse(parser: Parser<'a>) -> Result<Self> {
        let t = parser.parse::<kw::r#use>()?;
        //let names = parser.parse()?;
        //parser.parse::<kw::from>()?;
        /*let useNames = vec![];
        let star = Id::new("*", Span::n);
        let useName = UseName{other_name: star,our_name: star};
        useNames.push(useName);
        let names = UsedNames::List(useNames);
        */
        //wast::custom_reserved!(star = "*");
        //let t = parser.parse::<star>()?;

        let names = UsedNames::All(t.0);
...
impl<'a> Parse<'a> for UseName<'a> {
    fn parse(parser: Parser<'a>) -> Result<Self> {
        let (other_name, our_name) = if parser.peek::<Id>() {
